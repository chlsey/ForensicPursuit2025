...
init python:
    ...
    def toolbox_actions(item: str) -> None:
        ...
        #todo now define branch for every item in the toolbox
        if item == "phone":
            default_mouse = "phone"
        elif item == "phone_holder":
            if location == "phone_holder":
                default_mouse = "phone_holder"
        elif item == "phone_secure":
            default_mouse = "phone_secure"
    ...
    def inventory_actions(item: str) -> None:
        ...
        if item == "gin":
            ...
            if not microscope_setup and location == "stereo":
                default_mouse = "slide"
                hide_all_inventory()
...
init python:
    ...
    Takes in the item name as str to be removed as a parameter.
    """
    def removeToolboxItemByName(item_name: str):
        for item in toolbox_sprites:
            if item.type == item_name:
                removeToolboxItem(item)
                break
        repositionToolboxItems()
...
screen inspectItem(items):
    ...
    button:
        ...
        image "UI/inventory-icon-bg.png" align (0.5, 0.55) at half_size
        ...
        if items[0] == "lantern":
            ...
            # if "{}".format(items[0]) == "gin" or "{}".format(items[0]) == "fingerprint" or "{}".format(items[0]) == "label" or "{}".format(items[0]) == "baked_label":
            #     image "Inventory Items/inventory-{}.png".format(items[0]) align (0.5, 0.5) at Transform(zoom=1.5)
            # else:
